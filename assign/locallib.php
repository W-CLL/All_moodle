...
class assign
{
    ...
    public function view($action = '', $args = array())
    {
        ...
        if ($action == 'redirect') {
            ...
            //下载已经批注好的作业
        } else if ($action == 'downloadallpdf') {
            $o .= $this->download_allpdf();
            //
...
//下载批注过的pdf
    ...
    protected function download_allpdf()
    {
        global $CFG, $DB;
        ...
        $instanceid = $this->get_context()->id;
            ...
            if ($submission) {
                foreach ($grades as $grade) {
                    ...
                    foreach ($files as $file) {
                        ...
                        $filezip[$file->get_filename()] = $file;
                    }
                }
        ...
        if (count($filezip) == 0) {
            $header = new assign_header(
                ...
                '',
                $this->get_course_module()->id,
                get_string('downloadallpdf', 'assign')
            );
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('noannotate', 'assign'));
        ...
        } else if ($zipfile = $this->pack_files($filezip)) {
            \mod_assign\event\all_annotatedpdf_download::create_from_assign($this)->trigger();
    ...
    protected function view_grading_table()
    {
        ...
        //下载批注过的作业
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/assign/view.php?id=' . $cmid . '&action=downloadallpdf';
            $links[$downloadurl] = get_string('downloadallpdf', 'assign');
        }
        //
