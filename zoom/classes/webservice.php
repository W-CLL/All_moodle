...
class mod_zoom_webservice
{
     ...
     */
    public function get_meeting_webinar_info($id, $webinar, $required = true)
    {
        $url = ($webinar ? 'webinars/' : 'meetings/') . $id;
        $response = null;
        try {
    ...
    public function get_meeting_records($zoom)
    {
        list($inprogress, $available, $finished) = zoom_get_state($zoom);
        $meeting_time = strtotime($zoom->created_at);
        $mod_time = $zoom->start_time - 900;
        $status = $meeting_time > $mod_time;

        if ($inprogress) {  //进行中状态
            if ($status && $zoom->meeting_id != -1) {   //进行中 ，没有结束会议
                $data = array('type'=>'live');
                $url = 'metrics/meetings/'.$zoom->meeting_id.'/participants';
                try{
                    $response =  $this->_make_call($url, $data);
                 }catch (moodle_exception $error) {
                     
                    if(strpos($error->getMessage(),'Meeting ID is invalid or not end') == true){
                         return 0;
                    }else
                         throw $error;  
                }
        ...
        } else if ($finished) {//结束状态
            if($zoom->meeting_id != -1) {  //活动显示结束，会议室还存在
                $data = array('type'=>'past');
                $url = 'metrics/meetings/'.$zoom->meeting_id.'/participants';
                try {
                   $response =  $this->_make_call($url, $data);
                } catch (moodle_exception $error) {
